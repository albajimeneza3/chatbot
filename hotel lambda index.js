...
function getPersistenceAdapter() {
    ...
    const tableName = 'reservar_hotel_table';//-----------------------------------------------------------------------------------------------------
    if(isAlexaHosted()) {
...
const RegistrarHotelIntentHandler = {//-----------------------------------------------------------------------------------------------------
    canHandle(handlerInput) {
        return handlerInput.requestEnvelope.request.type === 'IntentRequest'
            && handlerInput.requestEnvelope.request.intent.name === 'RegistrarHotelIntent';//-----------------------------------------------------------------------------------------------------
    },
    handle(handlerInput) {
        const {attributesManager} = handlerInput;
        const requestAttributes = attributesManager.getRequestAttributes();
...
const PreguntarHotelIntentHandler = {//-----------------------------------------------------------------------------------------------------
    canHandle(handlerInput) {
        return handlerInput.requestEnvelope.request.type === 'IntentRequest'
            && handlerInput.requestEnvelope.request.intent.name === 'PreguntarHotelIntent';//-----------------------------------------------------------------------------------------------------
    },
    handle(handlerInput) {
        const {attributesManager} = handlerInput;
        const requestAttributes = attributesManager.getRequestAttributes();
        ...
        return handlerInput.responseBuilder
            .speak(requestAttributes.t('NOMBREHOTEL_MSG', message))
            .reprompt(requestAttributes.t('HELP_MSG'))
            .getResponse();
    }
};
//--------------------------------------------------------OOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOO
const SayHotelIntentHandler = {//-----------------------------------------------------------------------------------------------------
    canHandle(handlerInput) {
        return handlerInput.requestEnvelope.request.type === 'IntentRequest'
            && handlerInput.requestEnvelope.request.intent.name === 'SayHotelIntent';//-----------------------------------------------------------------------------------------------------
    },
    async handle(handlerInput) {
        ...
        const month = sessionAttributes['month'];
        const year = sessionAttributes['year'];
        const message = sessionAttributes['message'];
        ...
        if(day && month && year){    
            ...
            const nextHotel = moment(`${month}/${day}/${today.year()}`, "MM/DD/YYYY").tz(timezone).startOf('day');//-----------------------------------------------------------------------------------------------------
            if(today.isAfter(nextHotel)){//-----------------------------------------------------------------------------------------------------
                nextHotel.add('years', 1);//-----------------------------------------------------------------------------------------------------
            }
            const age = today.diff(wasBorn, 'years');
            const daysLeft = nextHotel.startOf('day').diff(today, 'days'); // same days returns 0 //-----------------------------------------------------------------------------------------------------
            speechText = requestAttributes.t('SAY_MSG', daysLeft, message);
            if(daysLeft === 0) {
...
const NombreHotelIntentHandler = {//-----------------------------------------------------------------------------------------------------
    canHandle(handlerInput) {
        return handlerInput.requestEnvelope.request.type === 'IntentRequest'
            && handlerInput.requestEnvelope.request.intent.name === 'NombreHotelIntent';//-----------------------------------------------------------------------------------------------------
    },
    async handle(handlerInput) {
        const {attributesManager} = handlerInput;
        const requestAttributes = attributesManager.getRequestAttributes();
        ...
        //} else {
            //speechText = requestAttributes.t('MISSING_MSG');
        //}
        speechText = requestAttributes.t('SAYHOTEL_MSG', message);
...
//---------------------------------------------------------------------------------
const OfertaHotelIntentHandler = {//-----------------------------------------------------------------------------------------------------
    canHandle(handlerInput) {
        return handlerInput.requestEnvelope.request.type === 'IntentRequest'
            && handlerInput.requestEnvelope.request.intent.name === 'OfertaHotelIntent';//-----------------------------------------------------------------------------------------------------
    },
    async handle(handlerInput) {
        const {attributesManager} = handlerInput;
        const requestAttributes = attributesManager.getRequestAttributes();
        ...
        speechText = requestAttributes.t('OFERTA_MSG');
...
exports.handler = Alexa.SkillBuilders.custom()
    .addRequestHandlers(
        ...
        RegistrarHotelIntentHandler,//-----------------------------------------------------------------------------------------------------
        SayHotelIntentHandler,//-----------------------------------------------------------------------------------------------------
        OfertaHotelIntentHandler,//------------------------------------------------
        NombreHotelIntentHandler,
        PreguntarHotelIntentHandler,
        HelpIntentHandler,
