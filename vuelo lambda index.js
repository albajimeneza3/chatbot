...
function getPersistenceAdapter() {
    ...
    const tableName = 'reservar_vuelo_table';
    if(isAlexaHosted()) {
...
const RegistrarVueloIntentHandler = {//-----------------------------------------------------------------------------------------------------
    canHandle(handlerInput) {
        return handlerInput.requestEnvelope.request.type === 'IntentRequest'
            && handlerInput.requestEnvelope.request.intent.name === 'RegistrarVueloIntent';//-----------------------------------------------------------------------------------------------------
    },
    handle(handlerInput) {
        const {attributesManager} = handlerInput;
        const requestAttributes = attributesManager.getRequestAttributes();
...
const SayVueloIntentHandler = {//-----------------------------------------------------------------------------------------------------
    canHandle(handlerInput) {
        return handlerInput.requestEnvelope.request.type === 'IntentRequest'
            && handlerInput.requestEnvelope.request.intent.name === 'SayVueloIntent';//-----------------------------------------------------------------------------------------------------
    },
    async handle(handlerInput) {
        ...
        if(day && month && year){    
            ...
            const nextVuelo = moment(`${month}/${day}/${today.year()}`, "MM/DD/YYYY").tz(timezone).startOf('day');//-----------------------------------------------------------------------------------------------------
            if(today.isAfter(nextVuelo)){//-----------------------------------------------------------------------------------------------------
                nextVuelo.add('years', 1);//-----------------------------------------------------------------------------------------------------
            }
            const age = today.diff(wasBorn, 'years');
            const daysLeft = nextVuelo.startOf('day').diff(today, 'days'); // same days returns 0 //-----------------------------------------------------------------------------------------------------
...
exports.handler = Alexa.SkillBuilders.custom()
    .addRequestHandlers(
        ...
        RegistrarVueloIntentHandler,//-----------------------------------------------------------------------------------------------------
        SayVueloIntentHandler,//-----------------------------------------------------------------------------------------------------
        HelpIntentHandler,
